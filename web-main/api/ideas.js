// Handler for the `/api/ideas` endpoint. Parses the request body safely, then uses
// the OpenAI API to generate three chatbot ideas in the requested language.
async function readJsonSafe(req) {
  // If the body has already been parsed to an object, return it directly.
  ...
  // Otherwise, read the request as a stream (sometimes Vercel provides a stream).
  const chunks = [];
